...
def find_lcfs_inside_circle(
    ...
    """
    Find the largest closed psi contour that:
      1) encloses the magnetic axis
      2) lies entirely inside the circle
            (R - R0)^2 + (Z - Z0)^2 <= a_max^2

    Returns
    -------
    lcfs : dict with keys
        "level", "vertices", "area", "axis"
    """

    axis_info = find_magnetic_axis(RR, ZZ, psi, axis=axis, mask=axis_mask)
    Raxis = axis_info["R"]
    Zaxis = axis_info["Z"]
    psiaxis = axis_info["psi"]

    if (Raxis - R0) ** 2 + (Zaxis - Z0) ** 2 > a_max**2:
        raise RuntimeError("Magnetic axis is outside the allowed circle")
    ...
    for level in levels:
        ...
        for c in enclosing:
            verts = c["vertices"]
            r2 = (verts[:, 0] - R0) ** 2 + (verts[:, 1] - Z0) ** 2
            if np.all(r2 <= a_max**2 + tol):
                inside_circle.append(c)

        if len(inside_circle) == 0:
            continue

        # keep the largest acceptable contour found so far
        last_good = max(inside_circle, key=lambda c: c["area"])

    if last_good is None:
        raise RuntimeError("ERROR: No closed flux surface around magnetic axis inside the circle")
...
if FREE:
    lcfs_target = find_lcfs_inside_circle(
            RR,
            ZZ,
            psi,
            axis=axis_type,
    ...
    psi_edge = lcfs_target["level"]

    
else:
    psi_on_LCFS,psimask = psi_on_circle(RR, ZZ,psi)
    psi_edge = np.mean(psi_on_LCFS)
...
for j in range(Niter):
    ...
    if FREE:
        #  update mask, psi_edge, and psi axis, and LCFS as largest closed contour
        ...
        # just update mask, psi_edge, and psi axis keeping LCFS == circle 
