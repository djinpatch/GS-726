...
def plot_rz_scalar(
    ...
    cf = ax.contourf(RR, ZZ, field_plot, levels=40, norm=norm, cmap = cmap)
...
def compute_gs_residual(
    RR, ZZ, psi, psimask, psi_axis, psi_edge, p0, F0, nu=2, src_scale=1.0
):
    R_vals = RR[:, 0]
    Z_vals = ZZ[0, :]
...
def plot_all_three(RR, ZZ, psi, psimask, psi_axis, psi_edge, axis_info, coils, p0, F0, nu, j,delta_list):
    ...
    if save_dir:
        fig.savefig(f"{save_dir}/number_{j:04d}.png", dpi=200)
...
for j in range(Niter):
    ...
    # update magnetic axis only every 5 iterations
    if j == 0 or (j + 1) % 5 == 0:
        axis_info = find_magnetic_axis(RR, ZZ, psi, axis=axis_type, mask=psimask)
        psi_axis = axis_info["psi"]
...
gs = compute_gs_residual(
    RR, ZZ, psi, psimask, psi_axis, psi_edge, p0, F0, nu=nu, src_scale=src_scale
)
